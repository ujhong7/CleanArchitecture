...
- **의존성 역전 원칙:**
    ...
    각 레이어는 프로토콜을 통해 상호작용하여 결합도를 낮추고, 테스트 가능성을 높였습니다.
    
    예를 들어, `UserListViewModel`은 `UserListUsecaseProtocol`을 통해 의존성을 주입받아 비즈니스 로직을 호출하며, 실제 구현체(`UserRepository` 또는 `MockUserListUsecase`)에 대해 알 필요가 없습니다.
    
    이를 통해 다음과 같은 장점을 구현하였습니다:
    
    - **레이어 간 의존성 최소화:**
        - `UserListUsecase`는 `UserRepository`와 상호작용하되, 구체적인 구현(`UserNetwork`, `UserCoreData`)을 알지 못하고 프로토콜로 호출합니다.
        - 이로써 각 레이어의 변경이 다른 레이어에 영향을 주지 않도록 설계하였습니다.
    - **테스트 용이성 향상:**
        - ViewModel 단위 테스트에서 의존성을 쉽게 Mock 객체(`MockUserListUsecase`)로 대체할 수 있도록 설계하였습니다.
        - 네트워크 응답이나 데이터베이스 상태를 시뮬레이션하여, 다양한 시나리오를 테스트할 수 있었습니다.
        - 예:
            - API 응답 데이터(Mock)를 기반으로 ViewModel이 `UserListCellData`로 변환하는 로직을 검증.
            - CoreData에서 반환된 즐겨찾기 데이터를 UI에 올바르게 매핑하는지 테스트.
    
    이 설계를 통해 **유연성과 확장성**을 확보함과 동시에 **테스트 가능한 구조**를 구현할 수 있었습니다.
    
- **테스트 코드:**
    
    ViewModel의 입력(Input)에 따른 출력(Output)이 의도한 대로 동작하는지 검증하기 위해 테스트 코드를 작성하였습니다.
    
    - **쿼리 결과가 `UserListCellData.user` 셀로 변환되는지 확인:**
        - 사용자 검색 쿼리 입력 시 API에서 반환된 `UserListItem` 데이터를 `UserListCellData.user`로 변환하는 과정을 검증하였습니다.
        - Mock 객체를 활용하여, API 응답 데이터를 시뮬레이션하였고, 변환된 데이터의 첫 번째 아이템이 올바르게 매핑되었는지 확인하였습니다.
        - 예: `UserListItem(id: 1, login: "user1")` → `.user` 타입 셀로 변환 후 `login` 정보가 "user1"로 매핑되는지 검증.
    - **즐겨찾기 데이터 검증:**
        - 탭 버튼이 즐겨찾기 모드로 변경될 때, CoreData에서 반환된 즐겨찾기 데이터가 UI에 적절히 매핑되는지 테스트하였습니다.
        - 즐겨찾기 목록 데이터가 알파벳 헤더와 함께 그룹화되어 변환되었는지 확인하고, 즐겨찾기 상태(`isFavorite`)가 올바르게 설정되었는지 검증하였습니다.
        - 예: 즐겨찾기 목록 `UserListItem(id: 1, login: "Ash")` → 헤더 "A"와 함께 `.user` 타입 셀로 변환 및 `isFavorite = true` 상태로 매핑되는지 테스트
    
    이 테스트 코드를 통해 ViewModel의 데이터 변환 로직이 올바르게 동작하는지 확인할 수 있었으며, 네트워크 및 데이터베이스 의존성을 제거하고 로직 검증에 집중할 수 있었습니다.
